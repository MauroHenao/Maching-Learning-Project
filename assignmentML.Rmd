...
```{r}
prediction_gbm <- predict(model_gbm, mytesting)
confusionMatrix(prediction_gbm, mytesting$classe)
```

The accuaracy of this model is 0.9953, higher than the other models.

##PREDICTING RESULTS WITH THE TEST DATA

The prediction is going to be made with the boosted regression model:

```{r}
prediction <- predict(model_gbm, test)
prediction
```
